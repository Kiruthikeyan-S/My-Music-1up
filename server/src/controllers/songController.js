...
export async function streamSong(req, res) {
  try {
    ...
    const filePath = song.audio_path;
...
export async function recordPlay(req, res) {
  try {
    ...
    db.execute('UPDATE songs SET play_count = play_count + 1 WHERE id = ?', [id]);

    // Record listening history if authenticated
    if (req.user) {
      db.execute(
        `INSERT INTO listening_history (user_id, song_id, duration_listened, played_at)
         VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
        [req.user.id, id, duration_listened]
      );
    }
